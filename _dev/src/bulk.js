	...
	w.imagify.bulk = {
		...
		init: function () {
			...
			this.hasMultipleRows =
				$('.imagify-bulk-table [name="group[]"]').length > 1;

			// Selectors (like the level selectors).
			$(".imagify-selector-button").on(
				"click.imagify",
			...
			$document.on("keypress.imagify click.imagify", this.closeSelectors);
		...
		getAjaxUrl: function (action, item) {
			var url =
				...
				"&action=" +
				imagifyBulk.ajaxActions[action];
		...
		hasBlockingError: function (displayErrorMessage) {
			displayErrorMessage =
				undefined !== displayErrorMessage && displayErrorMessage;
		...
		displayShareBox: function () {
			...
			$complete
				...
				.html(w.imagify.humanSize(globalSaved, 1));
			$complete
				.find(".imagify-ac-rt-total-original")
				.html(w.imagify.humanSize(this.globalOriginalSize, 1));
			$complete
				.find(".imagify-ac-chart")
				.attr("data-percent", Math.round(this.globalGain));
		...
		updateStats: function (data) {
			...
			if (w.imagify.bulk.charts.overview.donut.data) {
				donutData =
					w.imagify.bulk.charts.overview.donut.data.datasets[0].data;

				if (
					data.unoptimized_attachments === donutData[0] &&
					data.optimized_attachments === donutData[1] &&
					data.errors_attachments === donutData[2]
				) {
					return;
				}
			}
			...
			$(".imagify-unconsumed-bar")
				.css("width", data.unconsumed_quota + "%")
				.parent()
				.attr("class", data.quota_class);

			/**
			 * Global chart.
			 */
			$("#imagify-overview-chart-percent").html(
				data.optimized_attachments_percent + "<span>%</span>",
			);
			$(".imagify-total-percent").html(
				data.optimized_attachments_percent + "%",
			);
			...
			$("#imagify-total-optimized-attachments").html(
				data.already_optimized_attachments,
			);

			// The original bar.
			$("#imagify-original-bar")
				.find(".imagify-barnb")
				.html(data.original_human);

			// The optimized bar.
			$("#imagify-optimized-bar")
				.css("width", 100 - data.optimized_percent + "%")
				.find(".imagify-barnb")
				.html(data.optimized_human);

			// The Percent data.
			$("#imagify-total-optimized-attachments-pct").html(
				data.optimized_percent + "%",
			);
		},
		...
		syncSelectorFromRadio: function () {
			...
			$row.addClass("imagify-selector-current-value")
				.attr("aria-current", "true")
				.siblings(".imagify-selector-choice")
				.removeClass("imagify-selector-current-value")
				.attr("aria-current", "false");
			// Change the button text.
			$row.closest(".imagify-selector-list")
				.siblings(".imagify-selector-button")
				.find(".imagify-selector-current-value-info")
				.html($row.find("label").html());
		},
		...
		closeSelectors: function (e) {
			...
			w.imagify.bulk.closeLevelSelector(
		...
		maybeLaunchAllProcesses: function () {
			...
			swal({
				...
				.then(function () {
					var $row = $('.imagify-bulk-table [name="group[]"]:checked')
						.first()
						.closest(".imagify-row-folder-type");

					$.get(
						w.imagify.bulk.getAjaxUrl("bulkInfoSeen", {
			...
			w.imagify.bulk.folderTypesQueue.forEach(function (item) {
				...
				$.get(w.imagify.bulk.getAjaxUrl("bulkProcess", item))
					.done(function (response) {
						...
						// Success.
						if (response.success) {
							$row = $("#cb-select-" + item.groupID).closest(
							...
							$row.find(".imagify-cell-checkbox-loader")
								.removeClass("hidden")
								.attr("aria-hidden", "false");
							$row.find(".imagify-cell-checkbox-box")
								.addClass("hidden")
			...
			if (!$.isEmptyObject(w.imagify.bulk.status)) {
				...
				if (hasError) {
					if ("invalid-api-key" === hasError) {
						...
						"get-unoptimized-images" === hasError ||
						"consumed-all-data" === hasError
					) {
						errorArgs = {
					...
					if (
						Object.prototype.hasOwnProperty.call(
							imagifyBulk.labels.nothingToDoText,
							w.imagify.bulk.imagifyAction,
						)
					) {
						errorMsg =
							imagifyBulk.labels.nothingToDoText[
		...
		addStatsImagifybeat: function (e, data) {
			data[imagifyBulk.imagifybeatIDs.stats] = Object.keys(
				w.imagify.bulk.getFolderTypes(),
			);
		},
		...
		processStatsImagifybeat: function (e, data) {
			if (typeof data[imagifyBulk.imagifybeatIDs.stats] !== "undefined") {
				w.imagify.bulk.updateStats(
					data[imagifyBulk.imagifybeatIDs.stats],
				);
			}
		},
		...
		addQueueImagifybeat: function (e, data) {
			data[imagifyBulk.imagifybeatIDs.queue] = Object.values(
				w.imagify.bulk.getFolderTypes(),
			);
		},
		...
		processQueueImagifybeat: function (e, data) {
			...
			if (typeof data[imagifyBulk.imagifybeatIDs.queue] !== "undefined") {
				...
				if (false !== queue.result) {
					w.imagify.bulk.globalOriginalSize =
						queue.result.original_size;
					w.imagify.bulk.globalOptimizedSize =
						queue.result.optimized_size;
					w.imagify.bulk.globalOptimizedCount = queue.result.total;
					w.imagify.bulk.globalGain =
						(w.imagify.bulk.globalOptimizedSize * 100) /
						w.imagify.bulk.globalOriginalSize;
				}

				if (
					!w.imagify.bulk.processIsStopped &&
					w.imagify.bulk.hasBlockingError(true)
				) {
					$(w).trigger("queueEmpty.imagify");
					return;
				}
				...
				if (
					Object.prototype.hasOwnProperty.call(queue, "groups_data")
				) {
					Object.entries(queue.groups_data).forEach(function (item) {
						$row = $("[data-context=" + item[0] + "]");

						$row.children(".imagify-cell-count-optimized")
							.first()
							.html(item[1]["count-optimized"]);
						$row.children(".imagify-cell-count-errors")
							.first()
							.html(item[1]["count-errors"]);
						$row.children(".imagify-cell-optimized-size-size")
							.first()
							.html(item[1]["optimized-size"]);
						$row.children(".imagify-cell-original-size-size")
							.first()
							.html(item[1]["original-size"]);
					});
				}
				...
				$bar.css("width", queue.percentage + "%")
					.find(".percent")
					.html(queue.percentage + "%");
		...
		processRequirementsImagifybeat: function (e, data) {
			if (
				typeof data[imagifyBulk.imagifybeatIDs.requirements] ===
				"undefined"
			) {
				return;
			}
		...
		drawOverviewChart: function (data) {
			...
			if (!this.charts.overview.canvas) {
				this.charts.overview.canvas = d.getElementById(
					"imagify-overview-chart",
				);
			...
			this.charts.overview.data.datasets[0].data = [
				...
				parseInt(
					this.charts.overview.canvas.getAttribute("data-optimized"),
					10,
				),
				parseInt(
					this.charts.overview.canvas.getAttribute("data-errors"),
					10,
				),
			];
			...
			if (
				initData.datasets[0].data.reduce(function (a, b) {
					return a + b;
				}, 0) === 0
			) {
			...
			this.charts.overview.donut = new w.imagify.Chart(
			...
			$.each(initData.labels, function (i, label) {
				legend +=
					...
					initData.datasets[0].backgroundColor[i] +
					'"></span>' +
					label +
					"</li>";
			});
			...
			d.getElementById("imagify-overview-chart-legend").innerHTML =
				legend;
		},
		...
		drawShareChart: function () {
			...
			// Create new donut.
			this.charts.share.donut = new w.imagify.Chart(
				this.charts.share.canvas,
				{
					type: "doughnut",
					data: {

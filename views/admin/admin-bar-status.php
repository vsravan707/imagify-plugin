...
<div class="imagify-admin-bar-quota">
	<div class="imagify-abq-row">
		<?php if ($data['plan_with_quota']) : ?>
			<div class="imagify-meteo-icon"><?php echo $data['quota_icon']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
											?></div>
		<?php endif; ?>
	...
	<?php if ($data['plan_with_quota'] && $data['unconsumed_quota'] <= 20) : ?>
		<div class="imagify-upsell-admin-bar">
			...
			<p class="text-center center txt-center"><a class="imagify-upsell-admin-bar-button" href="<?php echo esc_url($data['upgrade_link']); ?>" target="_blank"><?php echo esc_html($data['button_text']); ?></a></p>

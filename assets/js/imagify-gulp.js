...
(function ($, d, w) {
	...
	w.imagify.Optimizer.prototype.run = function () {
		var chunkLength =
				this.prefixedMediaIDs.length > this.bufferSize
					? this.bufferSize
					: this.prefixedMediaIDs.length,
			i;
	...
	w.imagify.Optimizer.prototype.processedCallback = function (e, item) {
		...
		if (
			!item.mediaID ||
			typeof _this.files["_" + item.mediaID] === "undefined"
		) {
			return;
		}
	...
	w.imagify.Optimizer.prototype.processed = function (response) {
		...
		if (response.success && "already-optimized" !== response.status) {
			...
			this.globalPercent = (
				100 -
				(this.globalOptimizedSize / this.globalOptimizedSize) * 100
			).toFixed(2);
		}

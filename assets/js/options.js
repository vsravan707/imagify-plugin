...
(function ($, d, w, undefined) {
	 ...
	 * Fade CDN URL field.
	 */
	$('[name="imagify_settings[display_nextgen_method]"]')
		.on("change.imagify init.imagify", function (e) {
			if ("picture" === e.target.value) {
				$(e.target)
					.closest(".imagify-radio-group")
					.next(".imagify-options-line")
					.removeClass("imagify-faded");
			} else {
				$(e.target)
					.closest(".imagify-radio-group")
					.next(".imagify-options-line")
					.addClass("imagify-faded");
			}
		})
		.filter(":checked")
...
(function (w, d, $, undefined) {
	...
	$.getJSON(ajaxurl, w.imagifyUser).done(function (r) {
		if ($.isPlainObject(r) && r.success) {
			r.data.id = null;
			r.data.plan_id = null;
			r.data.is = [];
...
(function (w, d, $, undefined) {
	...
	function imagifyInsertFolderRow(value) {
		...
		$rows.each(function () {
			...
			if (
				"" !== thisValueTest &&
				valueTest.indexOf(thisValueTest) === 0
			) {
		...
		if ("" !== valueTest) {
			$rows.each(function () {
				...
				if (null !== prevPath && thisPath.indexOf(prevPath) === 0) {
					$this
						.find(".imagify-custom-folders-remove")
						.trigger("click.imagify");
				} else {
					prevPath = thisPath;
				}
			});
		...
		$.post(
			...
			.done(function (response) {
				...
				swal({
					...
					html:
						'<div class="imagify-swal-subtitle">' +
						imagifyOptions.labels.filesTreeSubTitle +
						'</div><div class="imagify-swal-content"><p class="imagify-folders-information"><i class="dashicons dashicons-info" aria-hidden="true"></i>' +
						imagifyOptions.labels.cleaningInfo +
						'</p><ul id="imagify-folders-tree" class="imagify-folders-tree">' +
						response.data +
						"</ul></div>",
					type: "",
					customClass:
	...
	$(d).on(
		"click.imagify",
		"#imagify-folders-tree [data-folder]",
		function () {
			var $button = $(this),
			...
			if (
				...
				$button.siblings(":checkbox").is(":checked")
			) {
				return;
			}
			...
			$("#imagify-custom-folders-selected")
				.find("input")
				.each(function () {
					selected.push(this.value);
				});
	...
	$("#imagify-custom-folders").on(
		"click.imagify",
		".imagify-custom-folders-remove",
		function () {
			var $row = $(this)
				.closest(".imagify-custom-folder-line")
				.addClass("imagify-will-remove");
...
(function (w, d, $, undefined) {
	...
	w.imagify.optionsBulk = {
		...
		init: function () {
			...
			if (
				false !== imagifyOptions.bulk.progress_next_gen.total &&
				false !== imagifyOptions.bulk.progress_next_gen.remaining
			) {
				// Reset properties.
		...
		addQueueImagifybeat: function (e, data) {
			data[imagifyOptions.bulk.imagifybeatIDs.progress] =
				imagifyOptions.bulk.contexts;
		},
		...
		processQueueImagifybeat: function (e, data) {
			...
			if (
				e.data.imagifyOptionsBulk &&
				typeof data[imagifyOptions.bulk.imagifybeatIDs.progress] ===
					"undefined"
			) {
				return;
			}
		...
		processRequirementsImagifybeat: function (e, data) {
			if (
				e.data.imagifyOptionsBulk &&
				typeof data[imagifyOptions.bulk.imagifybeatIDs.requirements] ===
					"undefined"
			) {
				return;
			}
		...
		processFinished: function () {
			...
			if (false !== this.error) {
				if ("invalid-api-key" === this.error) {
					...
					errorArgs = {
						title: imagifyOptions.bulk.labels
...
(function (w, d, $, undefined) {
	...
	$(".imagify-select-all").on("click.imagify", function () {
		...
		$btns
			.find(".imagify-select-all")
			.removeClass(inactive)
			.attr("aria-disabled", "false");
		$_this.addClass(inactive).attr("aria-disabled", "true");

		$group.find(".imagify-row-check").prop("checked", function () {
			var $this = $(this);
	...
	$(".imagify-check-group .imagify-row-check").on(
		"change.imagify",
		function () {

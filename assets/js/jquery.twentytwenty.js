	...
	$.fn.twentytwenty = function (options, callback) {
		...
		return this.each(function () {
			var sliderPct = options.handlePosition,
				...
				beforeDirection =
					"vertical" === sliderOrientation ? "down" : "left",
				afterDirection =
					"vertical" === sliderOrientation ? "up" : "right",
			...
			$container
				.children(".twentytwenty-overlay, .twentytwenty-handle")
				.remove();
			...
			$overlay.append(
				'<div class="twentytwenty-labels twentytwenty-before-label"><span class="twentytwenty-label-content">' +
					options.labelBefore +
					"</span></div>"
			);
			$overlay.append(
				'<div class="twentytwenty-labels twentytwenty-after-label"><span class="twentytwenty-label-content">' +
					options.labelAfter +
					"</span></div>"
			);
...
(function ($, d, w, undefined) {
	...
	var drawMeAChart = function (canvas) {
		...
		imagifyTwentyModal = function (options) {
			...
			if (
				...
				0 === settings.originalSize ||
				0 === settings.optimizedSize ||
				0 === settings.saving
			) {
				return "error";
			}
			...
			modalHtml =
				'<div id="' +
				settings.modalId +
				'" class="imagify-modal imagify-visual-comparison" aria-hidden="true">';
			/* eslint-disable indent */
			modalHtml += '<div class="imagify-modal-content loading">';
			modalHtml += '<div class="twentytwenty-container">';
			modalHtml +=
				'<img class="imagify-img-before" alt="" width="' +
				settings.width +
				'" height="' +
				settings.height +
				'">';
			modalHtml +=
				'<img class="imagify-img-after" alt="" width="' +
				settings.width +
				'" height="' +
				settings.height +
				'">';
			...
			modalHtml +=
				'<div class="imagify-c-level imagify-level-original go-left imagify-original-fize-size">';
			...
			modalHtml +=
				'<span class="value"><span class="imagify-chart"><span class="imagify-chart-container"><canvas id="imagify-consumption-chart-normal" width="15" height="15"></canvas></span></span><span class="imagify-chart-value">' +
				settings.saving +
				"</span>%</span>";
			modalHtml += "</p>";
			modalHtml += "</div>";
			modalHtml += "</div>";
			modalHtml +=
				'<button class="close-btn absolute" type="button"><i aria-hidden="true" class="dashicons dashicons-no-alt"></i><span class="screen-reader-text">' +
			...
			settings.trigger.on("click.imagify", function (e) {
				...
				$modal
					...
					.attr(
						"src",
						settings.optimizedUrl +
							(settings.optimizedUrl.indexOf("?") > 0
								? "&"
								: "?") +
							"v=" +
							Date.now()
				...
				checkLoad = setInterval(function () {
					...
					$tt.twentytwenty(
						...
						function () {
							...
							if (
								!$tt
									.closest(".imagify-modal-content")
									.hasClass("loaded")
							) {
								$tt.closest(".imagify-modal-content")
									.removeClass("loading")
									.addClass("loaded");
								drawMeAChart(
									$modal.find(
										".imagify-level-optimized .imagify-chart canvas"
									)
								);
							}
							...
							if (
								...
								$modal
									.find(".imagify-modal-content")
									.on("scroll.imagify", function () {
	...
	$(".imagify-visual-comparison-btn").on("click", function () {
		...
		$($(this).data("target"))
			.find(".imagify-modal-content")
			.css("width", $(w).outerWidth() * 0.95 + "px");

		if (
			$(".twentytwenty-container").length > 0 &&
			$(w).outerWidth() <= 800
		) {
			return;
		}
		...
		/* eslint-disable indent */
		ttBeforeButtons +=
			'<button type="button" class="imagify-comparison-original selected" data-img="original">' +
			labelOriginal +
			"</button>";
		ttBeforeButtons +=
			'<button type="button" class="imagify-comparison-normal" data-img="normal">' +
			labelNormal +
			"</button>";
		ttBeforeButtons +=
			'<button type="button" class="imagify-comparison-aggressive" data-img="aggressive">' +
			labelAggressive +
			"</button>";
		/* eslint-enable indent */
		...
		ttAfterButtons +=
			...
			"</button>";
		ttAfterButtons +=
			'<button type="button" class="imagify-comparison-ultra selected" data-img="ultra">' +
			labelUltra +
			"</button>";
		/* eslint-enable indent */
		ttAfterButtons += "</span>";
		...
		// Should be more locally integrated...
		image50 += $(".twentytwenty-left-buttons").lenght
			? ttBeforeButtons + ttAfterButtons
			: "";
				...
				function () {
					...
					if (
						...
						$tt.closest(".imagify-modal-content")
							...
							.addClass("loaded");
						drawMeAChart(
							$(".imagify-level-ultra .imagify-chart canvas")
						);
					}
				}
			);
		...
		$(".imagify-comparison-title").on(
			"click",
			".twentytwenty-duo-buttons button:not(.selected)",
			function (e) {
				var $this = $(this),
					$container = $this
						.closest(".imagify-comparison-title")
						.nextAll(".twentytwenty-wrapper")
						.find(".twentytwenty-container"),
					side = $this
						.closest(".twentytwenty-duo-buttons")
						.hasClass("twentytwenty-duo-left")
						? "left"
						: "right",
					$otherSide =
						"left" === side
							? $this
									.closest(".imagify-comparison-title")
									.find(".twentytwenty-duo-right")
							: $this
									.closest(".imagify-comparison-title")
									.find(".twentytwenty-duo-left"),
					$duo = $this
						.closest(".twentytwenty-duo-buttons")
						.find("button"),
				...
				if ("left" === side) {
					...
					$container
						.find(".img-" + image)
						.addClass("twentytwenty-before")
						.css("clip", clipStyles);
					$(
						".twentytwenty-before-label .twentytwenty-label-content"
					).text($container.data(image + "-label"));
					$(".imagify-c-level.go-left")
						.attr("aria-hidden", "true")
						.removeClass("go-left go-right");
					$(".imagify-level-" + image)
				...
				drawMeAChart(
					$(".imagify-level-" + image + " .imagify-chart canvas")
				);
			}
		);
	});
	 ...
	 */
	if (
		imagifyTTT.imageWidth &&
		$(".post-php .wp_attachment_image .thumbnail").length > 0
	) {
		var $oriParent = $(".post-php .wp_attachment_image"),
			oriSource = {
				src: $("#imagify-full-original").val(),
				size: $("#imagify-full-original-size").val(),
			},
			$optimizeBtn = $("#misc-publishing-actions").find(
				".misc-pub-imagify .button-primary"
			),
			filesize,
			saving;
		...
		if (imagifyTTT.imageWidth > imagifyTTT.widthLimit && oriSource.src) {
			...
			$('[id^="imgedit-open-btn-"]').before(
				'<button type="button" class="imagify-button-primary button-primary imagify-modal-trigger" data-target="#imagify-visual-comparison" id="imagify-start-comparison">' +
					imagifyTTT.labels.compare +
					"</button>"
			);
		...
		// Else put images next to next.
			...
			imagifyTTT.imageWidth < imagifyTTT.widthLimit &&
			oriSource.src
		) {
			// TODO
		}
		// If image has no backup.
			...
			$("#imagify-full-original").length > 0 &&
			"" === oriSource.src
		) {
			// do nothing ?
		}
		// In case image is not optimized.
			...
			if (
				$("#misc-publishing-actions").find(
					".misc-pub-imagify .button-primary"
				).length === 1
			) {
				$('[id^="imgedit-open-btn-"]').before(
					'<span class="spinner imagify-hidden"></span><a class="imagify-button-primary button-primary imagify-optimize-trigger" id="imagify-optimize-trigger" href="' +
						$optimizeBtn.attr("href") +
						'">' +
						imagifyTTT.labels.optimize +
						"</a>"
				);
	...
	if ($(".upload-php .imagify-compare-images").length > 0) {
		$(".imagify-compare-images").each(function () {
			var $this = $(this),
				...
				$datas = $this
					.closest("#post-" + id)
					.find(".column-imagify_optimized_file");
	...
	if ($(".upload-php").length > 0) {
			...
			imagifyContentInModal = function () {
				var tempTimer = setInterval(function () {
					...
					if (originalSrc) {
						// Trigger creation.
						$actions = $(
							".media-frame-content .attachment-actions"
						);

						$actions
							.find("#imagify-media-frame-comparison-btn")
							.remove();
						$actions.prepend(
							'<button type="button" class="imagify-button-primary button-primary imagify-modal-trigger" data-target="#imagify-comparison-modal" id="imagify-media-frame-comparison-btn">' +
								imagifyTTT.labels.compare +
								"</button>"
						);
						...
						// Get datas.
						$datas = $(
							".media-frame-content .compat-field-imagify"
						);
						...
						imagifyTwentyModal({
							...
							optimizedSize: $datas
								.find("#imagify_data_sum .big")
								.text(),
							saving: $datas.find(".imagify-chart-value").text(),
							modalAppendTo: $(
								".media-frame-content .thumbnail-image"
							),
		...
		$(".upload-php").on(
			"click",
			".media-frame.mode-grid .attachment, .edit-media-header .left, .edit-media-header .right",
			function () {
	...
	if ($("#imagify-files-list-form").length > 0) {
		var buildComparisonModal = function ($buttons) {
			$buttons.each(function () {
				var $this = $(this),
					...
					$datas = $this
						.closest("tr")
						.find(".column-optimization .imagify-data-item");
		 ...
		 */
		$(w)
			.on("comparisonprinted.imagify", function (e, id) {
				var $buttons;

				id = id || 0;

				if (id) {
					$buttons = $("#imagify-files-list-form").find(
						'.imagify-compare-images[data-id="' + id + '"]'
					);
				} else {
					$buttons = $("#imagify-files-list-form").find(
						".imagify-compare-images"
					);
				}

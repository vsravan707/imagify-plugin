...
(function ($, d, w, undefined) {
	...
	w.imagify.filesList = {
		...
		init: function () {
			...
			$(w)
				.on("canvasprinted.imagify", this.updateChart)
			...
			if ($processing.length) {
				...
				$processing
					.closest("tr")
					.find('.check-column [name="bulk_select[]"]')
					.each(function () {
		...
		updateChart: function (e, selector) {
			...
			$canvas
				.closest(".imagify-datas-list")
				.siblings(".imagify-datas-details")
				.hide();
		},
		...
		insertBulkActionTags: function () {
			var bulkActions =
				'<option value="imagify-bulk-optimize">' +
				w.imagifyFiles.labels.bulkActionsOptimize +
				"</option>";
			...
			if (w.imagifyFiles.backupOption || $(".file-has-backup").length) {
				...
				bulkActions +=
					'<option value="imagify-bulk-restore">' +
					w.imagifyFiles.labels.bulkActionsRestore +
					"</option>";
			}
		...
		processBulkAction: function (e) {
			...
			if (
				"imagify-bulk-optimize" !== value &&
				"imagify-bulk-restore" !== value &&
				"imagify-bulk-refresh-status" !== value
			) {
				return;
			}
			...
			$('input[name="bulk_select[]"]:checked')
				.closest("tr")
				.find(".button-imagify-" + action)
		...
		addToImagifybeat: function (e, data) {
			...
			$boxes.each(function () {
				var id = w.imagify.filesList.sanitizeId(this.value),
					context = w.imagifyFiles.context,
					locked = w.imagify.filesList.isItemLocked(context, id)
						? 1
						: 0;

				data[w.imagifyFiles.imagifybeatID][context] =
					data[w.imagifyFiles.imagifybeatID][context] || {};
...
(function ($, d, w, undefined) {
	...
	function lazyLoadThumbnailsCallback() {
		...
		$.each(lazyImages, function () {
			 ...
			 */
			if (
				imgBottom >= screenTopThresholded &&
				imgTop <= screenBottomThresholded
			) {

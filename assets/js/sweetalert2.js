...
(function (global, factory) {
	...
	var init = function init(params) {
		...
		if (!targetElement) {
			console.warn(
				"SweetAlert2: Can't find the target \"" + params.target + '"',
			);
	...
	var sweetHTML = (
		...
		'">Cancel</button>\n   </div>\n   <button type="button" class="' +
		swalClasses.close +
		'" aria-label="Close this dialog">\xD7</button>\n </div>\n'
	).replace(/(^|\n)\s*/g, "");
	...
	var elementByClass = function elementByClass(className) {
		return getContainer()
			? getContainer().querySelector("." + className)
			: null;
	};
			...
			if (params.animation) {
				switch (params.type) {
					...
					case "error":
						...
						addClass(
							icon.querySelector(".swal2-x-mark"),
	...
	var sweetAlert = function sweetAlert() {
		...
		return new Promise(function (resolve, reject) {
			...
			if (
				!window.onkeydown ||
				window.onkeydown.toString() !== handleKeyDown.toString()
			) {

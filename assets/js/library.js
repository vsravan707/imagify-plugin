(function ($, d, w, undefined) {
	...
	bulkOpt =
		...
		imagifyLibrary.labels.bulkActionsOptimize +
		"</option>";

	if ($(".button-imagify-optimize-missing-sizes").length) {
		// If we have items that have missing sizes.
		bulkOpt +=
			'<option value="imagify-bulk|optimize_missing_sizes">' +
			imagifyLibrary.labels.bulkActionsOptimizeMissingSizes +
			"</option>";
	}
	...
	if (imagifyLibrary.backupOption || $(".attachment-has-backup").length) {
		...
		bulkOpt +=
			...
			imagifyLibrary.labels.bulkActionsRestore +
			"</option>";
	}

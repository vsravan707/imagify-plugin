...
(function ($, d, w, undefined) {
	...
	$("#imagify-toggle-plan").change(function () {
		...
		$(".imagify-toggle-label")
			.eq(0)
			.css("color", isChecked ? "#c8ced5" : "#3b3f4a");
		$(".imagify-toggle-label")
			.eq(1)
			.css("color", isChecked ? "#3b3f4a" : "#c8ced5");
...
(function ($, d, w, undefined) {
	...
	imagifyModal = {
		...
		getHtmlPrice: function (content, period) {
			...
			if (typeof content !== "object") {
				...
				content[1] =
					content[1].length === 1
						? content[1] + "0"
						: ("" + content[1]).substring(0, 2);

				output =
					'<span class="imagify-price-big">' +
					content[0] +
					"</span> ";
				output +=
					'<span class="imagify-price-mini">.' +
					content[1] +
					"</span>";
			...
			output +=
				'<span aria-hidden="' +
				(period === "monthly" ? "false" : "true") +
				'" class="imagify-monthly">';
			output += '<span class="imagify-price-big">' + m[0] + "</span> ";
			output +=
				'<span class="imagify-price-mini">.' +
				(m[1].length === 1 ? m[1] + "0" : ("" + m[1]).substring(0, 2)) +
				"</span>";
			output += "</span> ";
			output +=
				'<span aria-hidden="' +
				(period === "yearly" ? "false" : "true") +
				'" class="imagify-yearly">';
			output += '<span class="imagify-price-big">' + y[0] + "</span> ";
			output +=
				'<span class="imagify-price-mini">.' +
				(y[1].length === 1 ? y[1] + "0" : ("" + y[1]).substring(0, 2)) +
				"</span>";
			output += "</span>";
		...
		getHtmlDiscountPrice: function (content, period) {
			...
			if (typeof content === "object") {
				...
				output +=
					...
					'" class="imagify-monthly">';
				output +=
					'<span class="imagify-price-discount-number">' +
					monthly +
					"</span>";
				output += "</span>";
				output +=
		...
		populateOffer: function ($offer, datas, type, classes) {
			...
			$offer
				.find(".imagify-number-block")
				.html(imagifyModal.getHtmlPrice(pcs, "monthly"));
			...
			// discount prices
			$offer
				.find(".imagify-price-block")
				.prev(".imagify-price-discount")
				.remove();
			if (
				promo.is_active &&
				"percentage" === promo.coupon_type &&
				...
				$offer
					.find(".imagify-price-block")
					.before(imagifyModal.getHtmlDiscountPrice(pcsd, "monthly"));
			}
			// Nb images.
			...
			$datas_c = $offer.find(".imagify-payment-btn-select-plan").length
				? $offer.find(".imagify-payment-btn-select-plan")
				: $offer;
				...
				$.post(
					...
					function (discount_response) {
						...
						$(".imagify-offer-selected")
							.removeClass("imagify-offer-selected")
							.find(".imagify-checkbox")
							.prop("checked", false);
						...
						if (null === offers.mo) {
							...
							// Show error message.
							$offers_block
								.closest(".imagify-modal-views")
								.find(".imagify-popin-message")
								.remove();
							$offers_block.after(
						...
						if (promo_datas.is_active) {
							if (promo_datas.applies_to instanceof Array) {
								...
								for (
									var plan_infos = 0;
									plan_infos < promo_datas.applies_to.length;
									plan_infos++
								) {
									plan_list.push(
							...
							discount =
								"percentage" === promo_datas.coupon_type
									? promo + "%"
									: "$" + promo;
							...
							// Populate banners.
							$banners
								.find(".imagify-promotion-number")
								.text(discount);
							$banners
								.find(".imagify-promotion-plan-name")
								.text(plan_names);
							$banners
								.find(".imagify-promotion-date")
								.text(date_end);
						}
						...
						if (0 === offers.mo.length) {
							...
							$.each(offers.mo, function (index, value) {
								...
								// Populate each offer.
								$tpl = $(mo_clone).clone();
								$tpl = imagifyModal.populateOffer(
									$tpl,
									value,
									"monthly",
									classes
		...
		iframeSetSrc: function (params) {
			...
			pay_src =
				pay_src + 0 + "/" + rt_monthly + "/" + rt_yearly + "/none/";
		...
		getPromoAppliesTo: function (promo) {
			...
			if (promo.applies_to instanceof Array) {
				...
				for (
					var plan_infos = 0;
					plan_infos < promo.applies_to.length;
					plan_infos++
				) {
	...
	$(d).on("modalClosed.imagify", ".imagify-payment-modal", function () {
		$(this)
			...
			.removeClass("imagify-success-viewing imagify-iframe-viewing");
	 ...
	 */
	imagifyModal.$modal.on(
		"click.imagify",
		".imagify-payment-btn-select-plan",
		function (e) {
			var checkout_datas;
